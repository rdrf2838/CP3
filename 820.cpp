...
const int VISITED = 1, UNVISITED = -1, INF = 1000000000, MAX_V = 110;
class UnionFind {
private:
	vi p, rank, setSize;
	int numSets;
...
vector<vi> AdjList;
int res[MAX_V][MAX_V], mf, f, s, t, c, u, v, w, n;
vi p;
...
int main() {
	...
	while (1) {
		...
		AdjList.clear(); memset(res, 0, sizeof(res));
		for (int i = 0; i < n; i++) {
		...
		while (c--) {
			...
			if (res[u][v] == 0)AdjList[u].push_back(v), AdjList[v].push_back(u);
			res[u][v] = res[v][u] += w;
		}
		mf = 0;
		while (1) {
